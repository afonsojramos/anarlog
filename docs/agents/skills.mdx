...
1. Prefer local MCP or `anarlog --json meetings --source local` when the agent can access your database.
...
The `Anarlog` plugin bundles the skill and an optional hosted Cloud MCP connection. [Install the Anarlog CLI](/installation) for local access; no Cloud login or Pro subscription is needed. For remote access, enable **Settings → Developers → Cloud API & Connectors** on a Pro account and connect through OAuth.
